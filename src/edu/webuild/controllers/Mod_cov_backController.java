 ...
 * @author manou
 */
public class Mod_cov_backController implements Initializable {
    ...
    private void Modify_btu(ActionEvent event) {
        ...
        if (nmbr_place > 7 || nmbr_place == 0 || nmbr_place < 0) {
            ...
            CoVoiturage v = new CoVoiturage(depart, destination, date_dep, nmbr_place);
            inter_co.modifierCoVoiturage(v, Integer.parseInt(Show_cov_backController.id_co));
            System.out.println(Integer.parseInt(Show_cov_backController.id_co));
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Information Dialog");
            alert.setHeaderText(null);
